...
namespace Maispace\MaiLocations\Service;
...
/**
 * Resolves PIDs where location records for a frontend page may be stored.
 *
 * Mirrors mai_seo RecordStorageResolver so plugins find page-local records
 * (e.g. smoke-test fixtures on pid 10001/10002) without flexform settings.
 */
final class LocationStoragePageResolver
{
     ...
     * @return list<int>
     */
    public function resolve(int $pageUid): array
    {
        if ($pageUid <= 0) {
            return [];
        }
        ...
        return array_values(array_unique(array_filter($pids, static fn(int $pid): bool => $pid > 0)));
    }
